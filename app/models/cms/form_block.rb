  ...
  class FormBlock < ActiveRecord::Base
    ...
    def self.inherited(subclass)
      ...
      subclass.class_eval do
        ...
        has_dynamic_attributes :class_name => "CmsFormBlockAttribute",
                               :foreign_key => "form_block_id",
                               :table_name => Namespacing.prefix("form_block_attributes"),
                               :relationship_name => :form_block_attributes

        acts_as_content_block(
            :versioned => false,
            :publishable => false,
            :renderable => {:instance_variable_name_for_view => "@form_block"})
        ...
        def self.helper_path
          "app/form_blocks/helpers/#{name.underscore}_helper.rb"
        end
    ...
    def self.types
      @types ||= ActiveSupport::Dependencies.autoload_paths.map do |d|
        if d =~ /app\/form_blocks/
          Dir["#{d}/*_form_block.rb"].map do |p|
    ...
    def self.get_subclass(type)
      raise "Unknown Form Block Type" unless types.map(&:name).include?(type)
      type.constantize
    end

    # For column in list
    def form_block_type_name
      type.titleize
    end

    def self.form
      "form_blocks/#{name.tableize.sub('_form_bocks', '')}/form"
    end
    ...
    def self.default_template_path
      @default_template_path ||= "form_blocks/#{name.tableize.sub('_form_blocks', '')}/render"
    end
